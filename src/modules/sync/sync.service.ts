...
export class SyncService {
  ...
  async syncUsers(): Promise<IServiceResult<SyncUserResultDto>> {
    ...
    try {
      ...
      const localUsersMap = new Map<number, IUser>(localUsersResult.data.map((user: IUser) => [user.trackerUid, user]));

      await Promise.all(
        fetchedUsersResult.data.map(async (fetchedUser) => {
          try {
            const localUser = localUsersMap.get(fetchedUser.uid);
            if (!localUser) {
              this.logger.warn(`User with trackerUid ${fetchedUser.uid} not found locally, creating new user`);
              const createUserResult = await this.userService.create({
                trackerUid: fetchedUser.uid,
              ...
              const needUpdate =
                localUser.trackerUid !== fetchedUser.uid ||
